...
const { client_auth, auth_header } = require('./utils/client-auth');
...
app.post('/', (req, res) => {
    ...
    client_auth(client_id, client_secret)
        .then((token) => {
            axios.get(endpoint, auth_header(token))
                .then((response) => {
...
app.get('/callback', function (req, res) {
    ...
    if (state === null || state !== storedState) {
        res.redirect(
            front_end_uri + qs.stringify({ error: "state_mismatch" })
        );
    } else {
        ...
        request.post(authOptions, function (error, response, body) {
            if (!error && response.statusCode === 200) {
                ...
                res.redirect(
                    front_end_uri + '/#' + qs.stringify({ error: 'invalid_token' })
                );
                console.log(error);
            }
        });
