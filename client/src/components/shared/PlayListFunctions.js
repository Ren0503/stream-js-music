...
import { LoginContext, PlayContext } from 'context';
...
function PlayButtonLarge({ loggedIn, playContext }) {
    ...
    if (loggedIn) {
        ...
        return (
            <button
                className="playButton no-outline"
                title="Play" data-tip='play'
                data-for='tooltipMain' data-event='click'
            >
                <Icon name="Play" height="28" />
            </button>
        );
...
function LikeButton({ follow, onFollow, loggedIn }) {
    if (loggedIn) {
        return (
            <button
                className={`likeButton ${follow ? 'noHover' : ''} no-outline`}
        ...
        return (
            <button
                ...
                title="Save to Your Library" data-tip='like'
                data-for='tooltipMain' data-event='click'
            >
                <Icon name='Heart' fill={follow} />
            </button>
...
function FollowButton({ follow, onFollow, loggedIn }) {
    if (loggedIn) {
        return (
            <button className="followButton no-outline" onClick={onFollow}>
        ...
        return (
            <button
                ...
                data-tip='follow' data-for='tooltipMain'
                data-event='click' onClick={() => console.log('hi')}
            >
                {follow ? 'following' : 'follow'}
            </button>
        );
